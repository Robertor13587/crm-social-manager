...
(async () => {
  ...
  const wfId = "2MgTHqALDviThVoB";

  const raw = await get(`${base}/api/v1/workflows/${wfId}`, { "X-N8N-API-KEY": apiKey });
  const w = JSON.parse(raw);

  const sql = w.nodes.find(n => n.name === "MySQL Insert Contact");
  if (sql) {
    sql.parameters.operation = "executeQuery";
    sql.parameters.query = "INSERT INTO wa_contacts (name, phone, tags, created_at) VALUES (\'{{$json.body.name}}\', \'{{$json.body.phone}}\', \'{{ $json.body.tags ? JSON.stringify($json.body.tags) : \"[]\" }}\', NOW()) ON DUPLICATE KEY UPDATE name = VALUES(name), tags = VALUES(tags);";
  }

  const retJson = w.nodes.find(n => n.name === "Return JSON");
  if (retJson) {
    retJson.parameters.jsCode = "const r = $json; const status = \"ok\"; const id = r.insertId || null; return [{ json: { status, data: { id, action: id ? \"created\" : \"updated\" } } }];";
  }
